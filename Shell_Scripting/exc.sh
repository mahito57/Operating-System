...
# Find and compile .c files in the root directory and subdirectories
find "$root_directory" -type f -name "*.c" -print0 | while IFS= read -r -d '' file; do
  # Get the parent directory of the .c file
  ...
  find "$test_case_directory" -type f -name "*.txt" | while IFS= read -r test_case; do
    ...
    ./main.out < "$test_case" > "out${counter}.txt"
